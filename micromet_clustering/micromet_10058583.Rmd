...
saveRDS(sfe, paste0(cur_sample, '/data/voyager_sfe_', cur_sample, '_niches_WIP.rds'))

sfe <- readRDS(paste0(cur_sample, '/data/voyager_sfe_', cur_sample, '_niches_WIP.rds'))
...
# run louvain
clusters <- igraph::cluster_louvain(g, resolution=1)
...
tmp %>% subset(cell_group == 'Vasculature')

min_size <- 15
#imm_keep <- tmp %>% subset(cell_group == 'Immune' & proportion < 0.5) %>% .$spatial_community
vasc_keep <- tmp %>% subset(cell_group == 'Vasculature' & proportion >= 0.05) %>% .$spatial_community
tumor_keep <- tmp %>% subset(cell_group == 'Tumor' & proportion >= 0.2) %>% .$spatial_community
size_keep <- as.numeric(names(which(table(sfe_sub$spatial_community) >= min_size)))


groups_keep <- Reduce(intersect, list(vasc_keep, tumor_keep, size_keep))
groups_keep
...
#table(sfe_sub$clusters_remove)

sfe_sub$spatial_community <- as.character(sfe_sub$spatial_community)
groups_keep <- as.character(groups_keep)

# plot the clusters in space
p <- plotSpatialFeature(
        sfe_sub[,sfe_sub$spatial_community %in% groups_keep], 'spatial_community', 
        colGeometryName = "cellSeg") +
...
table(sfe_sub[,sfe_sub$spatial_community %in% groups_keep]$spatial_community)
